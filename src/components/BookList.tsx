    ...
    <section>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {books.map((book: Omit<CartItem, 'quantity'>) => (
          <article key={book.id} onClick={() => {handleClick(book.id)}} className="bg-white p-4 border border-[#eea36d] rounded-lg shadow-lg hover:shadow-[#d1baab] transition-shadow duration-600 cursor-pointer">
